...
describe("Playlist Component", () => {
  ...
  test("no activities defined", () => {
    ...
    const noActs = getByText(
      "You currently have no activities in your playlist"
    );
  ...
  test("The playlist renders activity list carousel", () => {
    ...
    const { debug, getByText } = render(
      <MemoryRouter>
        <Playlist
          store={store}
          session={"d4cd70a6-15e0-11ea-8d71-362b9e155667"}
          playlistToRender={mockedPlaylistData}
        />
      </MemoryRouter>
    );
  ...
  test("clicking on the card re-directs you to the activity", () => {
    ...
    expect(Location.pathname).toBe(
      `/view-playlist/${mockedPlaylistData[0].id}`
    );
  });
});
